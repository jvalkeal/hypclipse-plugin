 ...
 * Optionally, a button can be added after the text. The attached
 * listener reacts to all the events. Entring new text makes
 * the entry 'dirty', but only when 'commit' is called 
 * is 'valueChanged' method called (and only if 'dirty' flag is set).
	...
	private void keyReleaseOccured(KeyEvent e) {
		if (e.character == '\r') {
			...
			if (!fValue.equals(fText.getText()))
				fText.setText(fValue != null ? fValue : ""); // restore old 
			fDirty = false;
		}
		if (fListener != null)
