...
public abstract class XMLInputContext extends InputContext {
	 ...
	 * @param input
	 */
	public XMLInputContext(HQDEFormEditor editor, IEditorInput input, boolean primary) {
		...
		if (textNode.getOffset() > -1) {
			...
			if (parent.getOffset() > -1) {
				...
				for (int i = 0; i < parent.getLineIndent(); i++)
					buffer.append(" ");
				buffer.append("   " + getWritableTextNodeString(textNode));
...
//			printDeletionRange(offset, length);
	...
	/*
	 * (non-Javadoc)
	 * @see org.hyperic.hypclipse.internal.context.InputContext#flushModel(org.eclipse.jface.text.IDocument)
	 */
