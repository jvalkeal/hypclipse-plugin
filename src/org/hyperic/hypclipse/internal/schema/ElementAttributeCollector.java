 ...
 * 
 * We are trying to find all attributes. Attribute is always on form 
 * of AttributeExp. However, it's possible to use annotation to encapsulate
 * metadata to block elements, AttributeExp may be encapsulated inside
 * AnnonationExp expression.
 * 
 * Either AnnotationExp or AttributeExp is stored during collect method.
 * It's up to user to check which expression is collected, since expressions
 * are stored as Expression object.
 */
public class ElementAttributeCollector implements ExpressionVisitorVoid {
    ...
    public void onOther( OtherExp exp ) {
    	if(exp instanceof AnnotationExp) {
    		if(exp.exp instanceof AttributeExp) {
    			result.add(exp);
    			((AttributeExp)exp.exp).exp.visit(this);
    		} else {
    			exp.exp.visit(this);
    		}
    	}
